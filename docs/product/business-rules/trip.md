...
| **BR-TRIP-005** | 추천 모드 및 알고리즘 | 방장이 후보 일정을 요청할 때 | **wave 2 — 4모드 전부.** **(1) 기본** — 불참률·부분참석·불확실인원·연차 4개 평가항목 동일 가중치, **(2) 모두 참석** — 불참률·부분참석 가중치를 크게(BR-TRIP-011), **(3) 휴가 아끼기** — 연차 가중치를 크게, **(4) 확실하게 가기** — 불확실인원 가중치를 크게. 평가항목별 패널티×모드 가중치 합산(`최종점수=100-Σ(패널티×가중치)`)으로 각 모드 TOP 3 산출. 근거 텍스트 대신 참석률·부분참여·불확실·연차 통계를 카드로 노출 | 잘못된 모드·데이터 부족 시 400 |
