...
| **BR-TRIP-005** | 추천 모드 및 알고리즘 | 방장이 후보 일정을 요청할 때 | **wave 2 — 4모드 전부**(기본/모두 참석/휴가 아끼기/확실하게 가기). 평가항목별 패널티×모드 가중치로 각 모드 TOP 3 산출, 근거는 참석률·부분참여·불확실·연차 통계 카드로 노출. 패널티 구간표·모드별 가중치·공식은 [`trip-recommendation-algorithm.md`](../../specs/trip/trip-recommendation-algorithm.md) — 여기서 중복 서술하지 않음 | 잘못된 모드·데이터 부족 시 400 |
