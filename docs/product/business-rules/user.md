...
| **BR-USER-004** | 회원 탈퇴 | 탈퇴 요청 | 확인 후 탈퇴 — `ONGOING`인 방에 활성 OWNER/MEMBER 있으면 차단(방 삭제/방 나가기 선행). CONFIRMED/TERMINATED/CANCELED는 차단 안 함 | [`user-account-withdrawal.md`](../../specs/user-account-withdrawal.md)(#48) · [`trip-member-leave.md`](../../specs/trip-member-leave.md)(#47) |
...
| **BR-USER-008** | 전역 일정 | 일정·`is_all_free` 변경 | **ONGOING** 방 달력에만 동일(live). **CONFIRMED/TERMINATED**는 snapshot 고정·읽기 전용 — [`trip-schedule-snapshot.md`](../../specs/trip-schedule-snapshot.md) (#38 **Approved**) | — |
...
| 2026-07-23 | BR-USER-004 `[미정]`(진행 중 방) 해소 — 활성 OWNER/MEMBER 방 있으면 탈퇴 차단 (`#47`·`#48`) |
