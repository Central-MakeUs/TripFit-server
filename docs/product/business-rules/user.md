...
| **BR-USER-004** | 회원 탈퇴 | 탈퇴 요청 | 확인 후 탈퇴 — 차단 없이 자동 cascade. 참여 중인 모든 방에서 자동 나가기(MEMBER) 또는 소유한 모든 방 자동 삭제(OWNER) 후 탈퇴 처리. 전 상태(`ONGOING`/`CONFIRMED`/`TERMINATED`) 적용 | [`user-account-withdrawal.md`](../../specs/user-account-withdrawal.md) · [`trip-member-leave.md`](../../specs/trip-member-leave.md) · 정책 근거 `#47` |
