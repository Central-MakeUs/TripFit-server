...
상세 인증 API·검증 방식: [`docs/specs/auth-social-login-mvp.md`](../specs/auth-social-login-mvp.md)
...
- 백엔드: `AppleTokenVerifier` + `POST /api/v1/auth/login` (`provider: APPLE`) — [`auth-social-login-mvp.md`](../specs/auth-social-login-mvp.md)
...
1. **지금 (MVP · Phase 1)**: [`auth-social-login-mvp.md`](../specs/auth-social-login-mvp.md) — login/refresh/logout (RTR·Redis 없음)
2. **Phase 1 이후**: [`auth-token-lifecycle-p2.md`](../specs/auth-token-lifecycle-p2.md) — **RTR + Redis 확정** (access blacklist/whitelist `[미정]`)
3. **스토어 직전**: Apple S2S Notification webhook, 딥링크, 앱 버전 호환
4. **런칭 후**: 푸시, 분석, 결제 등 — 각각 스펙 + decisions
...
| `specs/auth-social-login-mvp.md` | Phase 1 소셜 로그인·JWT |
| `specs/auth-token-lifecycle-p2.md` | P2 RTR + Redis (Draft) |
| `specs/auth-apple-server-notifications.md` | Apple 계정 변경 S2S webhook (스토어 제출 전) |
| `decisions/001-auth-mobile-token-verification.md` | 모바일 토큰 검증 + JWT (안 B) |
| `decisions/004-auth-token-lifecycle-p2.md` | RTR + Redis 확정 |
