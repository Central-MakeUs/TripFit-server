...
>
> **이 문서는 정책·엣지 케이스 시나리오용이다.** 명령형 API 호출 순서·에러 매핑의 SSOT는 [`docs/specs/trip/trip-join-schedule-gate.md`](../../specs/trip/trip-join-schedule-gate.md) 등 해당 스펙이다.
...
4. 플로우 중 이탈 → **`SCHEDULE_PENDING`으로 남음**(자리 유지) · 재진입 시 일정 플로우부터(`join`은 멱등). **미입장 상태에서는 스스로 나갈 수 없다**(403 `SCHEDULE_ACTIVATION_REQUIRED`) — 자리를 비우려면 일정 입력을 마쳐 입장한 뒤 나가거나, **방장이 내보내야** 한다 (`#122`)
...
| 푸시 · 카카오·링크 공유 | #21·#19 MVP 출시 (~~내보내기 #20 Nice~~ — 구현 완료 · ~~hold #35~~ — `#114`로 폐지) |
