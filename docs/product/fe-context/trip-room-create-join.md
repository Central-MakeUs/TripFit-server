...
방장과 멤버는 서로 다른 경로로 `RESPONDED`(방 입장 가능 상태)에 도달한다고 가정하고 구현하라.

- **방장**: `POST /trips` 호출 즉시 멤버 row가 생기지만 상태는 `JOINED`다. 별도로 `POST .../schedule/confirm`을 호출해야 `RESPONDED`가 된다. **2단계**로 구현하라.
- **멤버**: 일정을 다 입력한 뒤 `POST /trips/join`을 호출하는 순간 바로 `RESPONDED`로 등록된다. 멤버에게는 `JOINED`라는 중간 상태가 존재한다고 가정하지 마라. **1단계**로 구현하라.
...
STEP 3. POST /api/v1/trips/{tripId}/schedule/confirm
  → 이 호출로 JOINED → RESPONDED 전이가 일어난다고 가정하라
...
- 일정을 0건도 입력하지 않고(Skip) confirm을 호출해도 에러로 처리하지 마라 — 서버가 자동으로 `isAllFree=true`로 채운 뒤 통과시킨다. Skip 버튼을 눌러도 confirm 호출 자체는 항상 정상 진행되도록 만들어라.
- `403 SCHEDULE_ENTRY_REQUIRED`에 대한 에러 UI를 만들되, 우선순위를 낮게 잡아라 — 정상 플로우에서는 서버가 자동으로 조건을 채우므로 실사용 중 거의 발생하지 않는 방어용 케이스다. 이 에러를 재현하려고 별도 테스트 케이스를 만들 필요는 없다.
- confirm을 이미 `RESPONDED`인 상태에서 다시 호출해도 에러 없이 같은 상세가 온다고 가정하라(idempotent) — 중복 호출 방지 로직을 서버 대신 프론트에서 구현할 필요 없다.

### 방장 이탈 후 재진입 처리 — 다음 두 가지를 반드시 구현하라

1. 홈 목록(`GET /trips`)에서 `myMemberStatus`가 `JOINED`인 카드를 발견하면, 탭 시 상세 페이지로 보내지 말고 STEP 2(정기 일정 입력)로 라우팅하라. 이 카드에는 `inviteCode`가 없으므로 공유 버튼을 노출하지 마라.
2. 위 라우팅을 실수로 빠뜨렸을 때를 대비해 방어 코드도 넣어라: `GET /trips/{tripId}` 호출이 `403 SCHEDULE_CONFIRM_REQUIRED`를 반환하면, 무조건 정기 일정 입력 화면으로 리다이렉트하라.
...
STEP 3. POST /api/v1/trips/join { "inviteCode": "A2B3C4" }
  → 이 한 번의 호출로 멤버 row가 role=MEMBER, status=RESPONDED로 바로 생성된다고 가정하라
...
- 이미 `RESPONDED`인 멤버가 같은 초대 링크를 다시 열었을 때는 `join`을 그대로 호출해도 된다 — 에러 없이 상세가 오므로(idempotent), "이미 참여한 방인지" 사전 체크 로직을 따로 만들지 마라.
...
| `GET` | `/api/v1/trips` | JWT | 홈 목록 카드. `myMemberStatus`로 `JOINED`/`RESPONDED` 분기하라. `inviteCode` 없음 |
...
`confirm`/`join`/상세 조회 공통 응답 형태(`TripDetailResponse`) 예시:
...
| 403 | `SCHEDULE_CONFIRM_REQUIRED` | 방장이 `JOINED` 상태로 상세·방 안 API 호출 → 일정 입력 화면으로 라우팅하라 |
| 403 | `SCHEDULE_ENTRY_REQUIRED` | 방 입장 조건 미충족 — 정상 플로우에서는 자동 처리되므로 실사용 중 거의 발생하지 않는다 |
...
| | 방장 | 멤버 |
|---|---|---|
| 멤버 row 생성 시점 | 방 생성 즉시(`JOINED`) | `join` 호출 시점(`RESPONDED`로 바로) |
| 완료 API | `POST .../schedule/confirm` | `POST /trips/join` |
| 중간 이탈 시 라우팅 | `JOINED` 카드 → 일정 입력 화면으로 | row 없음 → 초대 링크부터 |
| 홈 카드 탭 분기 필요 여부 | **필요** — `myMemberStatus=JOINED`면 상세 대신 일정 입력으로 | 불필요 — 멤버는 항상 `RESPONDED`로만 존재 |
