...
- **상태:** 확정
- **날짜:** 2026-06-30
- **관련:** [`docs/specs/auth-social-login-mvp.md`](../specs/auth-social-login-mvp.md)
...
MVP에서는 핵심 여행방·일정 플로우 검증이 우선이고, 계정 연결(BR-USER-003)은 P2다.
...
- **계정 연결 불가**: 한 provider = 한 user. Kakao·Google 통합은 P2 `user_identity` 스펙 필요
...
- **토큰 Lifecycle P2**: **RTR + Redis 도입 확정** — access blacklist/whitelist는 `[미정]`. [`004-auth-token-lifecycle-p2.md`](004-auth-token-lifecycle-p2.md), [`auth-token-lifecycle-p2.md`](../specs/auth-token-lifecycle-p2.md). Phase 1은 DB refresh + stateless access(`jti`)만
- **캘린더 연동**: 로그인 OAuth와 scope·token 저장이 다름 — MVP+1 별도 스펙
- **DB 변경 자유도**: 인증 범위 내 허용이나, `user` rename·Identity 분리는 본 결정 범위 밖

## 후속 작업

- [ ] `docs/specs/auth-social-login-mvp.md` 기준 구현 (`POST /api/v1/auth/login` 단일 엔드포인트)
...
- [ ] Phase 1 완료 후: [`004-auth-token-lifecycle-p2.md`](004-auth-token-lifecycle-p2.md) — RTR + Redis (access 전략 합의)
- [ ] P2: 계정 연결 시 identity 스펙 검토
