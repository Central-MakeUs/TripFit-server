...
- **상태:** 확정 (MVP 출시 = A안, 출시 이후 = B안 예정)
- **날짜:** 2026-07-07
- **관련:**
  - [`docs/specs/auth/auth-social-login.md`](../specs/auth/auth-social-login.md) — MVP 출시 login·user upsert
  - [`docs/specs/user/user-profile-image-s3-mirror.md`](../specs/user/user-profile-image-s3-mirror.md) — 출시 이후 B안 Draft
  - Issue #1 (MVP 출시 구현), Issue #9 (출시 이후 B안)

## 맥락

소셜 로그인 시 Google·Kakao는 프로필 이미지 **외부 URL**을 제공한다. Apple id_token에는 프로필 이미지가 없다.

TripFit `user.profile_image_url`에 무엇을 저장할지, MVP(MVP 출시)와 이후(출시 이후) 전략을 나눠 확정한다.

## 결정

### MVP 출시 — **A안: provider URL 그대로 DB 저장** (확정)
...
**이유:** 구현·운영이 가장 단순하고, MVP 출시 목표(로그인 → JWT → 프로필 노출)에 충분하다.

### 출시 이후 — **B안: 서버 미러링 → TripFit S3 URL 저장** (예정)
...
| 대안 | 장점 | 단점 | 채택 |
|------|------|------|------|
| **A — URL passthrough (MVP 출시)** | 코드·인프라 없음, 즉시 사용 | URL 깨짐·provider 정책 변경에 취약 | **✓ MVP 출시** |
| **B — S3 미러링 (출시 이후)** | 안정적 CDN URL, 통제 가능 | S3·비동기·실패 처리·비용 | **✓ 출시 이후 예정** |
| 클라이언트가 S3 presigned upload | 서버 부하 적음 | 앱·웹 계약 복잡, 소셜 URL → 업로드 UX | ✗ |
| DB에 바이너리(BLOB) | 외부 URL 없음 | DB 비대, CDN 불리 | ✗ |

## 트레이드오프 · MVP 출시 리스크 (A안)

- Google/Kakao CDN URL은 **언제든 무효화**될 수 있다 → 깨진 아바타 가능.
- URL에 **세션·토큰 쿼리**가 포함될 수 있다 → 로그·Referer 유출 주의 (출시 이후 B안에서 완화).
- Apple은 MVP 출시에서도 `profileImageUrl` null이 일반적.

## 코드·스키마 (MVP 출시)

- 컬럼명 `profile_image_url` **유지** — 출시 이후 B안에서도 TripFit URL 문자열을 같은 컬럼에 저장 (rename 불필요).
...
- [x] MVP 출시: A안 코드·스펙·ERD 반영
- [ ] 출시 이후: B안 스펙 확정 + S3·IAM·Issue #9 구현
...
| 날짜 | 변경 |
|------|------|
| 2026-07-07 | 초안 — A안 MVP 출시 확정, B안 출시 이후 예정 |
