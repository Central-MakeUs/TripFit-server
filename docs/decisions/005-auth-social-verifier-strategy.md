...
2. **provider별 검증 로직 격리** — Google/Kakao/Apple의 HTTP·JWT·설정 차이를 `auth/oauth/` 아래에만 둔다.
...
| 역할 | 타입 | 패키지 | 책임 |
|------|------|--------|------|
| Strategy (인터페이스) | `SocialTokenVerifier` | `auth/oauth/` | `verify(token)` → `OAuthProfile` |
...
| Adapter DTO | `OAuthProfile` | `auth/oauth/` | verifier → service 경계. `provider`, `providerUserId`, `email`, `nickname`, `profileImageUrl` (nullable, **A안** provider URL passthrough — [`006`](006-profile-image-url-storage.md)) |
...
| 파일 | 역할 |
|------|------|
| `auth/oauth/SocialTokenVerifier.java` | Strategy 인터페이스 |
| `auth/oauth/SocialTokenVerifierRegistry.java` | Registry |
...
| 날짜 | 변경 |
|------|------|
| 2026-07-23 | **패키지 경로 정정** — `auth/client/` → `auth/oauth/` (003의 2026-07-20 리네임 반영, 본 문서가 갱신되지 않아 뒤처져 있었음) |
