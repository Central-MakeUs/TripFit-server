...
- **관련:**
  - [`docs/decisions/001-auth-mobile-token-verification.md`](001-auth-mobile-token-verification.md) — 안 B (MVP 출시)
...
MVP 출시(MVP)은 `POST /auth/login`, `/refresh`, `/logout`과 DB 기반 opaque refresh token, stateless access JWT(2h)로 시작한다.
...
2. **Redis를 인증 인프라에 도입한다.**
   ...
   - MVP 출시 구현 시 Redis **미포함**. 출시 이후 스펙 Approved 후 추가
...
- **채택 이유:** Whitelist는 로그인·갱신마다 쓰기가 발생하고 활성 세션 전체를 Redis가 들고 있어야 하는데, `010-redis-infra.md`의 EC2 D는 이중화 없는 **단일 인스턴스**라 Redis 장애 시 "허용 목록 확인 불가 = 전체 로그인 마비"(fail-closed)가 된다. Blacklist는 같은 장애 상황에서 "즉시 무효화 기능만 잠깐 못 씀, 로그인 자체는 MVP 출시처럼 정상 동작"(fail-open)으로 훨씬 안전하게 열화된다. 데이터량·쓰기 부하도 Blacklist가 구조적으로 적다(로그아웃한 사람만 기록, 로그인마다 기록 아님)
- 상세 키 설계·JwtFilter 동작은 [`docs/specs/auth/auth-token-rotation.md`](../specs/auth/auth-token-rotation.md)가 SSOT

## MVP 출시 → 출시 이후 관계

전환 상세(API 응답 변경·데이터 모델·환경 변수)는 [`docs/specs/auth/auth-token-rotation.md`](../specs/auth/auth-token-rotation.md)가 SSOT. 요지만: MVP 출시 코드는 출시 이후를 막지 않도록 **`jti`**, **`family_id`**, **`TokenRevocationChecker` interface(NoOp)** 를 이미 포함한다 — 출시 이후는 RTR 로직 + Redis 연동을 **추가**할 뿐 MVP 출시 API 계약(`POST /auth/login`)은 바꾸지 않는다.
...
| MVP 출시부터 RTR | **미채택** | MVP 일정·프론트 계약 단순화 우선 |
| Redis 없이 access만 stateless | MVP 출시만 | logout 후 access 2h 유효 — 출시 이후에 Redis로 해소 |
| Refresh rotation 없음 | **미채택** | refresh 탈취 시 장기 세션 위험 |

## 트레이드오프 · 후속 리스크

- **프론트 계약 변경:** 출시 이후 refresh 응답에 `refreshToken` 필드 추가 — 클라이언트 저장 로직 필요
...
- [x] MVP 출시 구현 완료 후 출시 이후 착수 (JwtFilter + Redis + RTR) — 코드·테스트 완료, 배포 검증 대기
