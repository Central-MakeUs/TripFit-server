...
사용자가 "SOLID·객체지향 4대 원칙·`@RequiredArgsConstructor` 미적용·`@Setter` 남발"을 지적하며 `/safe-refactor`로 개선을 요청. 그런데 `@Setter` 남발은 이미 auth·trip·user-schedule·notification 4개 도메인 감사에서 "저장소 전역 컨벤션이라 도메인 하나만 좁혀 고치면 일관성이 깨진다"는 이유로 반복해서 C로 미뤄져 있던 사안이었다(`core-followup.md` "반복 주제" 기준 충족) — 그래서 도메인별 순차 진행 대신, 먼저 사용자와 함께 **저장소 전체에 적용할 정책**을 정하고 한 번에 적용했다.
