...
새 기능 스펙(`docs/specs/`)이 "앞으로 만들 것"이라면, 이 폴더는 **이미 있는 것을 점검한 결과와 그 반영 이력**이다. 세 종류가 있다.

| 종류 | 대상 | 위치 | 만드는 주체 |
|------|------|------|-------------|
| **코드 감사** | 도메인별 구현 코드 — API 계약·비즈니스 로직은 바꾸지 않고 내부 품질만 개선 | `docs/audits/{domain}/` | [`safe-refactor`](../../.claude/skills/safe-refactor/SKILL.md) 스킬 (B 트랙) |
| **하네스 회고** | 에이전트를 통제하는 장치 — 규칙·스킬·에이전트·훅이 놓친 것 | [`harness-retro.md`](harness-retro.md) (이 폴더 루트) | G4 회고 게이트 · [`retro`](../../.claude/skills/retro/SKILL.md) 스킬 |

하네스 회고는 도메인 축에 속하지 않으므로 **도메인 폴더 안이 아니라 루트에 둔다.** `{domain}/` 하위는 `safe-refactor` 산출물 전용으로 유지해, 폴더만 보고 무엇이 들어 있는지 예측할 수 있게 한다.

세 종류가 보는 대상이 다르다. 코드 감사는 **구현 코드**, 문서 품질 감사는 **문서가 읽히는가**, 하네스 회고는 **에이전트를 통제하는 장치 자체**를 점검한다.
...
로컬 실험 단계로 시작했고, 6개 도메인 모두 3차 라운드까지 완료된 지금도 여전히 GitHub 이슈 트래킹 없이 진행하고 있다 — `safe-refactor` 감사·구현은 각 단계 승인 게이트로 대신하며 별도 이슈 번호를 붙이지 않는다. 실제로 PR을 올릴 단계가 되면 `CONTRIBUTING.md`의 이슈 번호 기반 브랜치 규칙이 다시 필요하다.
