...
| `senior-spring-backend-reviewer` (커밋 1개) | 약 75,000 | diff + 주변 코드 — 적정 |
...
여기서 나온 규약은 `workflow-tools.md` "서브에이전트 호출 규약" 절에 세 줄로 고정했습니다 — 대상은 경로 목록으로만 넘기고, 전문 읽기는 신규 파일에만 지시하고, 10개를 넘으면 나눠 부릅니다. 같은 규약을 지켜 다시 부른 호출은 약 112,000 토큰으로 줄었습니다.
