...
- **참여 완료:** 방장·멤버 **동일** — 방 진입(방장 `POST /trips` · 멤버 `POST /trips/join`) 시 `SCHEDULE_PENDING` → 일정 확인 → `POST .../activate` 후 `ACTIVE` (2026-09-13 `#114` — 이전에는 멤버만 join 한 번으로 바로 `ACTIVE`였다). 방 안 API는 `ACTIVE` (구 전역 `canEnterRoom` 조건은 2026-08-18 `#113`으로 삭제) ([#39](https://github.com/Central-MakeUs/TripFit-server/issues/39))
...
| 문서 | 내용 |
|------|------|
| `docs/product/flows/trip-create-join-guide.md` | 생성·참여 상세(빠른 요약·시나리오·정책, SCHEDULE_PENDING→activate 포함) |
...
| ID | 항목 | 결정 | 확정일 |
|----|------|------|--------|
| **D1** | 참여 완료 | 방장 create=`SCHEDULE_PENDING` → `POST .../activate`=`ACTIVE`. 멤버 join=`ACTIVE`. **submit 없음** — [#39](https://github.com/Central-MakeUs/TripFit-server/issues/39) | 2026-07-21 |
| **D2** | 그룹 일정 조회 | **T1** — `members/schedule-calendar`(정기+개별 합친 달력) **OpenAPI 공개**. 기간=`startRange`~`endRange`. **#38** CONFIRMED/EXPIRED snapshot · **#37** 마이페이지 today+2년·칩 | 2026-07-21 |
...
방장 `SCHEDULE_PENDING` — **방 입장 전**. 공유용 `inviteCode`는 **포함하지 않음** (DB에는 발급). activate → `ACTIVE` 후 `GET/…` 상세에서 노출.
...
trip `startRange`~`endRange`(**희망 기간 = 조회 기간**, #37 C2/C3). 멤버 **전원** × 정기+개별 합친 값 (S1·R2=A · #17).
