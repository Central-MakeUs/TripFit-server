...
| Method | Path | 변경 |
|--------|------|------|
| `POST` | `/api/v1/trips/join` | 멤버를 `SCHEDULE_PENDING`으로 생성(호출 시점: 일정 플로우 진입 **전**) · 비관적 락으로 정원 체크 · 응답 축소 |
| `POST` | `/api/v1/trips/{tripId}/activate` | 계약 불변 — 참여자도 호출한다는 점만 문서화 |
...
- [ ] 이미 `SCHEDULE_PENDING`인 사람이 join을 다시 호출하면 예외(기존 동작 회귀)
- [ ] `activate` 시 일정 0건이면 `is_all_free`가 켜지고, join 시점에는 켜지지 않음
