...
| 신규 참여 (join) | ✗ | **2026-09-13 `#114` amend** — `join`은 초대 링크를 연 시점(멤버가 `SCHEDULE_PENDING`으로 생김)이라 방에 실질적 변화가 없다. 실제 참여 완료인 `activate`가 touch하므로 이벤트가 사라지는 게 아니라 시점이 정확해진다 |
