...
| 계층 | 담당 | 하지 않음 |
|------|------|-----------|
| **프론트** | 제목·설명 · 템플릿 조립 · 카카오「공유하기」/「링크 복사」 · **공유 UI = 방장 ∧ ACTIVE** | SCHEDULE_PENDING 화면에서 공유 노출 |
| **백엔드** | `inviteCode`는 **ACTIVE 게이트된 상세**에서만 API 노출 · create(SCHEDULE_PENDING) 응답에 `inviteCode` **미포함** | 템플릿 DB 저장 · 카카오 대행 발송 · **공유 전용 신규 Must API 없음** |
...
1. **S-1 + S-2:** 공유 UI = **방장** 이고 **`ACTIVE`(방 입장 후)** 만
2. 제목·설명 입력 — 클라 로컬 · 서버 미저장
3. 공유하기 / 링크 복사 — 클라 SDK

## 공유 모드 · 서버 데이터 매핑 (확정 — 신규 API 없음)

### A. 확정 전 — 방 초대 (`ONGOING` · 방장 ACTIVE)
...
- [x] create 응답에 `inviteCode` 없음 · 상세(ACTIVE)에만 있음
- [x] C = 상세 + members로 충분 · **신규 share API 없음**
- [x] D3 URL · 6자 `inviteCode` 유지
- [x] SCHEDULE_PENDING/ACTIVE·공유 계약 Swagger·glossary·trip-room-api 필독 절
- [x] `./gradlew test`
...
| 2026-07-22 | **S-1** · C-1 · FE 템플릿 · SCHEDULE_PENDING=방장 전용 |
