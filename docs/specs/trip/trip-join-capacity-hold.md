...
> wave: **4**
> 상태: **Superseded** (2026-08-18, `#114`) — 구현·배포됐던 기능이지만 [`trip-join-schedule-gate.md`](trip-join-schedule-gate.md) J-4가 Redis hold를 **DB 비관적 락**으로 대체하며 엔드포인트 2개와 Redis Lua·TTL 코드를 전부 삭제했다. 아래 본문은 이력으로만 읽을 것 — 현행 정원 보장은 `POST /trips/join`이 `trip` 행을 잠근 채 카운트+INSERT를 한 트랜잭션에서 처리한다
