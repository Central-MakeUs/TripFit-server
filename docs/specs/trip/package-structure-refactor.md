...
> 상태: Implemented (2026-08-06 구현, `#100` → PR #101로 2026-08-08 `main` merge 완료. `./gradlew build` 그린, `oasdiff breaking`/`diff` 0건, dev-login 기반 API 스모크 테스트(생성~참여~추천~피드백~확정) 실통과로 검증) — **2026-08-26 포트/어댑터 부분 폐기**(구현체가 항상 1개뿐이라 실질적 이득 없어 concrete 클래스 직접 주입으로 되돌림, `docs/decisions/003-architecture-guide.md` 결정 11 폐기·[`docs/audits/trip/refactor-log.md`](../../audits/trip/refactor-log.md) 참고). 이 문서는 **당시 설계 배경의 이력 기록**으로 유지하고 본문은 amend하지 않는다 — 현재 구조는 위 refactor-log가 SSOT.
...
**참고 (2026-08-02 재확인):** `TripServiceSupport`는 최근 `safe-refactor round2` 커밋에서 이미 배치 조회 메서드(`resolveMergedSchedules(List<UUID>, ...)`, `GoogleCalendarService.findBusyDaysByUserIds(...)`)를 갖추게 됐다 — 포트 인터페이스 설계 시 이 배치 시그니처를 그대로 채택하고, 단건 버전을 새로 포트에 노출하지 않는다.
...
**남은 절차 (사용자 확인 필요):** 커밋 분할·`gh pr create`는 아직 진행하지 않음 — `.claude/rules/core-workflow.md` "새 PR 생성은 항상 먼저 확인" 절.
...
| 2026-08-26 (포트/어댑터 폐기) | `safe-refactor` 스킬로 trip 도메인 재감사 — `SchedulePort`·`GoogleCalendarPort`·`UserDirectoryPort` 3개 전부 구현체가 항상 1개뿐이었고(갈아끼우는 용도 없음), `user` 도메인이 다른 경로로 이미 `trip` 타입에 의존하고 있어 순환 의존 차단 효과도 없었다는 게 재확인됨. 사용자 결정으로 포트 3개(`trip/port/out/`) + `GoogleCalendarPortAdapter` 삭제, 남은 두 어댑터는 `implements`만 제거해 concrete 클래스(`ScheduleAvailabilityService`·`UserDirectoryService`로 개명)로 유지 — trip 쪽 5개 호출부는 인터페이스 대신 이 concrete 클래스를 직접 주입받는다. 동시에 "구글 busy 조회 → 일정 병합" 2단계 호출이 3곳에 복제돼 있던 문제(순서를 컴파일러가 아니라 주석으로만 보장)를 `ScheduleAvailabilityService.resolveAvailability(...)` 하나로 통합해 제거. `TripCommandService`가 `support.findUser`와 자기 `userDirectoryPort` 필드를 모두 쓰던 이중 접근 경로도 `TripServiceSupport.requireProfileNameComplete(...)` 위임으로 정리. `./gradlew test` 전체 통과, `oasdiff` diff 0건. decision 003 결정 11 폐기 amend. 상세: [`docs/audits/trip/refactor-log.md`](../../audits/trip/refactor-log.md) |
