...
3. **근본 원인(필드 위치가 `User`가 아니라 `RegularSchedule`)은 별도 이슈 `#52`로 분리** — API 계약이 바뀌는 breaking change라 프론트 담당자 부재중에는 진행하지 않기로 함(2026-08-15 사용자 결정). "첫 번째 행 기준" 로직은 2026-08-16 `#107` 구현에서 `RegularSchedule.policySource`로 옮겨, 연차·휴일 정보가 같은 대표 행을 쓰도록 단일 SSOT가 됐다(그 메서드에 제거 TODO 표시).
...
- [x] **응답 참여자 판정(2026-07-30 구현 확정 — 최초 초안에서 정정):** **이 방의 ACTIVE 멤버 전원**이 응답 참여자다. 별도의 "resolve 결과가 비어 있으면 미응답 제외" 필터는 두지 않는다 — `ACTIVE`가 되려면 반드시 방 입장 일정 확인 플로우를 거치므로, 일정을 하나도 넣지 않은 멤버는 "아직 답하지 않은 사람"이 아니라 **"넣을 일정이 없어 전부 가능한 사람"**이다. 따라서 resolve 결과가 비어 있는 것과 "미응답"은 다른 의미다 (2026-08-18 `#113` — 이 근거를 `User.isAllFree` 컬럼으로 표현하던 서술을 교체. **판정 로직·결과는 불변**). 이 방식이면 응답 참여자 0명은 자연히 발생하지 않는다(방장은 항상 ACTIVE 상태로만 추천을 생성할 수 있음)
...
| 7 | **연차 자동 전환 대상 (2026-08-15 amend)** — "정기 근무만으로 IMPOSSIBLE"인 슬롯만 전환 가능. 개별 일정 override·구글 캘린더 busy로 IMPOSSIBLE인 슬롯은 연차로 전환 불가(연차는 근무만 대체, 개별 일정·구글 일정은 대체 불가) — "연차/반차 자동 반영 규칙" 절 |
...
- [x] **(2026-08-15 amend) 개별 일정·구글 busy는 연차로 전환 안 됨** — 개별 일정 override(`classifyMembers_personalScheduleBlocksVacationConversion`)·구글 캘린더 busy(`classifyMembers_googleBusyBlocksVacationConversion`) 둘 다 연차 예산이 충분해도 전환되지 않음을 확인
...
- [x] **(2026-08-15 amend #4) 개별 일정으로 막힌 저녁은 예산이 있어도 못 엶** — 저녁 전용 근무(19~23시)에 개별 일정(예: 밤 결혼식)까지 겹치면 연차 예산이 충분해도 전환 후보에서 제외되는지 확인 (`classifyMembers_eveningBlockedByPersonalSchedule_cannotBeOpenedEvenWithBudget`)
