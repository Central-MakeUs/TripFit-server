...
| [`auth-token-rotation.md`](auth/auth-token-rotation.md) | 4 | **Superseded** (2026-09-15, `#4` Closed) — 전제 2개(refresh MySQL SSOT · access Blacklist)가 `auth-refresh-redis-cookie`로 뒤집힘. RTR 개념 설명 이력으로만 유지 | RTR (구 설계: MySQL + Redis Blacklist) | auth-social-login · decision 004·010 |
...
| [`schedule-slot-override.md`](user-schedule/schedule-slot-override.md) | 2 | **Implemented** (#67 Closed) | S1(개별 전체 대체) → O1(슬롯 단위 오버라이드) 전환 | schedule-calendar-resolve.md |
| [`schedule-holiday-rest.md`](user-schedule/schedule-holiday-rest.md) | 2 | **Implemented** (#107 Closed) | `holidayRest` 근무일 판정 반영 — 공공데이터포털 특일정보 API + Redis 캐싱, 달력·추천 공통 | schedule-calendar-resolve (A4) · decision 011 |
...
| [`vacation-policy-user-migration.md`](user-schedule/vacation-policy-user-migration.md) | 4 | **Implemented** (`#52` Closed, PR [#111](https://github.com/Central-MakeUs/TripFit-server/pull/111) merge 완료) | 연차·휴일 정보를 `RegularSchedule` → `User`로 이동 (스키마 리팩터, 정책 불변) | schedule-unified · trip-recommendation-algorithm |
| [`pre-schedule-entry-flow.md`](user-schedule/pre-schedule-entry-flow.md) | 2 | **Implemented** (2026-08-19, `#113`·`#114` Closed) | 사전 일정 입력 최초/갱신 판정을 `사전 신청일` 단일 마커로 확정 · 회원가입에서 사전 일정 제거 · `activate` 입력 완료 게이트 · 정기 일정 전체 삭제 API | schedule-participation-onboarding · trip-join-schedule-gate · vacation-policy-user-migration |
...
| [`trip-schedule-calendar-window.md`](trip/trip-schedule-calendar-window.md) | 2 | **Implemented** (#37 Closed) | 마이페이지 today+2년 · 방=희망 기간 · ONGOING 칩 | #17 · #12 |
...
| [`trip-schedule-snapshot.md`](trip/trip-schedule-snapshot.md) | 2 | **Implemented** (#38 Closed) | CONFIRMED/EXPIRED snapshot · R-model A | #27 · #17 · #37 |
| [`trip-recommendation.md`](trip/trip-recommendation.md) | 2 | **Implemented** (#13 Closed) | 추천 API 설계·요청/응답 껍데기·DTO·ERD·상태 전이·확정·취소 (계산 로직 제외) | #12 · #17 · #22 |
...
| [`trip-join-capacity-hold.md`](trip/trip-join-capacity-hold.md) | 4 | **Superseded** (2026-08-19, `#114` — hold 완전 폐지·DB 비관적 락으로 대체) | join 정원 hold/TTL — 이력 문서 | trip-join-schedule-gate |
| [`trip-join-schedule-gate.md`](trip/trip-join-schedule-gate.md) | 2 | **Implemented** (2026-08-19, `#113`+`#114`) · **BR-USER-006·007 개정 / 011 삭제 포함** | 참여자 `join`을 `SCHEDULE_PENDING`으로 앞당겨 방 입장 일정 확인을 서버가 강제 · hold→DB 비관적 락 대체 · 전역 입장 게이트(`is_all_free`) 삭제 | #22 · #39 · #110 |
| [`package-structure-refactor.md`](trip/package-structure-refactor.md) | 4 | **Implemented** (`#100` Closed, PR #101) · **2026-08-26 포트/어댑터 부분 폐기**(구현체가 항상 1개라 concrete 직접 주입으로 회귀) — 현재 구조 SSOT는 [`audits/trip/refactor-log.md`](../audits/trip/refactor-log.md) | trip 도메인 패키지 재설계 — 이력 문서 | decision 003 |
...
| 스펙 | wave | 상태 | 범위 | 선행 |
|------|------|------|------|------|
| [`notification.md`](notification/notification.md) | 3 | **Implemented** (#21 Closed) | FCM 푸시 · BR-NOTI-001~005·009 · BR-USER-005 · 알림센터 | #12 · #13 · 참여 완료 정의 |
...
| **#52** | 연차·휴일 정보를 `RegularSchedule`→`User`로 이동(스키마 리팩토링, wave 4) — `#105` 임시 우회("가장 먼저 등록된 행 기준")의 근본 수정. **이슈 번호 재사용**(구 "auth-dev-stub-verifier"는 `dev-login` 삭제로 폐기, 이 chore가 대체) | Closed |
