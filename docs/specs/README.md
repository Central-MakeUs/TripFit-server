...
| [`vacation-policy-user-migration.md`](user-schedule/vacation-policy-user-migration.md) | 4 | **Approved** (2026-08-16, `#52`) · PR [#111](https://github.com/Central-MakeUs/TripFit-server/pull/111) 구현 완료·merge 대기 | 연차·휴일 정보를 `RegularSchedule` → `User`로 이동 (스키마 리팩터, 정책 불변) | schedule-unified · trip-recommendation-algorithm |
| [`pre-schedule-entry-flow.md`](user-schedule/pre-schedule-entry-flow.md) | 2 | **Approved** (2026-08-19, 사용자 승인) · 구현 완료·merge 대기 | 사전 일정 입력 최초/갱신 판정을 `사전 신청일` 단일 마커로 확정 · 회원가입에서 사전 일정 제거 · `activate` 입력 완료 게이트 · 정기 일정 전체 삭제 API | schedule-participation-onboarding · trip-join-schedule-gate · vacation-policy-user-migration |
...
| 스펙 | wave | 상태 | 범위 | 선행 |
|------|------|------|------|------|
| [`schedule-participation-onboarding.md`](trip/schedule-participation-onboarding.md) | 2 | **Implemented** (#22 · **#39 amend**) · **Wave 1→2 이동**(2026-08-03, trip 참여 흐름) | join 게이트 · `SCHEDULE_PENDING`→activate · submit 폐기 · hold→#35 | user-onboarding |
...
| [`trip-calendar-window-pre-join.md`](trip/trip-calendar-window-pre-join.md) | 4 | **Draft (범위 축소, 2026-08-19)** ([#110](https://github.com/Central-MakeUs/TripFit-server/issues/110)) · 승인 전 구현 금지 | 본래 증상은 `#114`가 해소 — 남은 범위는 `GET`/`PATCH` 윈도우 검증 비대칭 하나 | #37 · #53 · #114 |
