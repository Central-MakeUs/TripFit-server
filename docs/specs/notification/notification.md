...
| **D11** | NOTI-002 "마지막 참여자" 판정 기준 | **ACTIVE 멤버 수가 여행방 정원(BR-TRIP-001, 1~10)에 도달한 순간** — `POST .../activate`에서 판정한다. 자리만 차지한 `SCHEDULE_PENDING` 멤버는 제외 (2026-08-18 `#114` — 이전에는 join 즉시 ACTIVE라 '정원 도달=전원 제출'로 봤다) |
