...
> MVP: 해당 없음 (Google Calendar 자체가 MVP 출시 Out of scope)
> 관련 BR: 해당 없음
...
> 선행: [`google-calendar-oauth.md`](google-calendar-oauth.md)(MVP 출시, FE 미착수)
...
- Calendar가 아직 FE에 구현된 적이 없어(목업, MVP 출시 미착수) 지금 당장 급한 작업은 아니다 — Calendar를 실제로 FE에 붙이는 시점에 이 분리를 함께 진행한다.
...
- FE의 Calendar 연동 화면 실제 OAuth 연결(로그인과 다른 Client ID로 요청 전환) — `google-calendar-oauth.md`(MVP 출시) 본연의 범위, 별도 저장소
