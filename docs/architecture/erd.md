...
**제약:** user당 **0..N행**. 행 수는 방 입장 판정에 쓰이지 않는다 — 구 전역 입장 조건은 `#113`(2026-08-18)으로 삭제됐고, 사전 일정 입력 완료 판정은 `users.vacation_apply_period` 하나가 담당한다(2026-08-19). soft delete 없음. 연차·휴일 정보는 `#52`(2026-08-16)로 `users`로 이동 — 위 `users` 절 참고.
