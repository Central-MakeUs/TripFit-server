...
# oasdiff로 base/revised OpenAPI 스펙을 비교해 breaking change가 있으면
# Discord #frontend 웹훅으로 알림을 보내고 실패(exit 1)한다. 없으면 조용히 통과한다.
...
if [[ "$BREAKING_COUNT" -eq 0 ]]; then
  log "breaking change 없음 — 통과"
  exit 0
fi

log "breaking change ${BREAKING_COUNT}건 감지 — Discord 알림 발송"
...
if [[ "${#CHANGES_TEXT}" -gt 1000 ]]; then
  CHANGES_TEXT="${CHANGES_TEXT:0:1000}...\n(전체 목록은 GitHub Actions 로그 참고)"
fi
