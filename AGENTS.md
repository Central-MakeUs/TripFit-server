...
기획·검증 기준을 먼저 고정하고, 그에 맞춰 구현합니다. **기획**은 `docs/product/`, **기능 설계**는 `docs/specs/`, **아키텍처 선택**(필요할 때만)은 `docs/decisions/`에 둡니다. 작업 크기(S/M/T)에 따라 스펙·이슈 생략 여부를 나눕니다 (`harness-workflow` 규칙). 구현 후 `./gradlew test`와 PR·CI로 검증합니다.
...
| `docs/README.md` | 문서 인덱스 (기획·아키텍처·스펙) |
...
| `docs/product/platform.md` | React 앱·스토어 런칭·API 전제 (Agent 필수 맥락) |
...
| `docs/decisions/` | 기술 결정 메모 (아키텍처·인프라, 필요 시만) |
| `deploy/README.md` | Docker·EC2 배포 (app/mysql 분리) |
| `.github/README.md` | Issue·PR·Milestone 워크플로 |
| `.dev/README.md` | 임시 세션 로그 (장기 문서는 `docs/`로) |
| `.cursor/README.md` | Cursor 설정 구조·규칙·훅 설명 |
