...
@Schema(
    description = """
        ...
        이름 그대로 "일정 확인 대기중"과 "방 활동 가능"을 뜻하며, 방장에게만 일정 확인 절차가 있어
        SCHEDULE_PENDING을 거친다 — 일반 멤버(초대 참여)는 이 값 없이 바로 ACTIVE가 된다.
        """)
public enum TripMemberStatus {
  @Schema(
      description = """
          의미: 방장이 방을 만들었지만, 아직 이 방 일정 확인을 끝내지 않아 방 안 입장이 막힌 상태.

          언제: POST /trips 직후 ~ POST /trips/{tripId}/activate 전.
