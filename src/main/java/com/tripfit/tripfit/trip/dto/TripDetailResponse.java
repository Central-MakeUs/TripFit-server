...
public record TripDetailResponse(
    ...
    @Schema(
        description = "여행방 진행 상태(effectiveStatus). end_range 경과 등 반영된 화면 표시용") TripStatus status,

    @Schema(
        description = "초대 코드 (6자). 방 입장(ACTIVE) 후 상세에서만 노출. 공유용. create(SCHEDULE_PENDING) 응답에는 없음") String inviteCode,
    ...
    @Schema(
        description = "본인 멤버십 상태. SCHEDULE_PENDING=방장 create 직후만(입장·공유 불가), ACTIVE=방장 activate 후·멤버 join 시(입장 가능)") TripMemberStatus myMemberStatus,
