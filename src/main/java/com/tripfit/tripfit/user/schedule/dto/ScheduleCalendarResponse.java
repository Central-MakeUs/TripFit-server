...
public record ScheduleCalendarResponse(
    @Schema(description = "조회 시작 날짜 (조회 윈도우 내)", example = "2026-08-01") LocalDate startDate,

    @Schema(description = "조회 종료 날짜 (조회 윈도우 내)", example = "2026-08-07") LocalDate endDate,

    @Schema(description = "합친 값이 있는 날짜만 포함 (sparse)") List<CalendarDayResponse> days
) {

  @Schema(description = "날짜 1일의 정기+개별을 합친 슬롯")
  public record CalendarDayResponse(
      @Schema(description = "날짜", example = "2026-08-03") LocalDate date,

      @Schema(description = "오전 [00:00, 13:00) 합친 값",
          example = "IMPOSSIBLE") ScheduleStatus morningStatus,

      @Schema(description = "오후 [13:00, 18:00) 합친 값",
          example = "IMPOSSIBLE") ScheduleStatus afternoonStatus,

      @Schema(description = "저녁 [18:00, 24:00) 합친 값",
