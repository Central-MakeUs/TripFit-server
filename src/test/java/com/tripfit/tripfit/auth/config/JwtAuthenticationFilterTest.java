...
class JwtAuthenticationFilterTest {
	...
	void doFilterInternal_validToken_setsSecurityContext() throws Exception {
		...
		when(tokenRevocationChecker.isRevoked(org.mockito.ArgumentMatchers.anyString())).thenReturn(false);

		filter.doFilterInternal(request, response, filterChain);

		verify(filterChain).doFilter(request, response);
		assertThat(SecurityContextHolder.getContext().getAuthentication()).isInstanceOf(JwtAuthentication.class);
