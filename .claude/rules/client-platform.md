...
| **멤버십 SCHEDULE_PENDING/ACTIVE** | **SCHEDULE_PENDING=방장 create 직후만**(멤버 아님). 멤버 join=`ACTIVE` 즉시. 공유=방장∧ACTIVE. create에 `inviteCode` 없음 — 상세는 `glossary.md` · `trip-room-api` 필독 절 · Trip/Trip Members `@Operation` 설명에만. Swagger 전역 `Info`·`@Tag`는 **요약 한 줄 + 필독 포인터만**, 상태 전이 상세 서술 금지 |
...
- Swagger `description`/`@Schema`는 FE 가독성 규칙(`spring-boot-java` OpenAPI 절: 목적·호출 시점·전제·결과 섹션) 준수 — 이슈 번호·BR/스펙 ID 금지
