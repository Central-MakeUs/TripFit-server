...
# harness-workflow.md STOP §3 — no prod-preserved data; schema SSOT is JPA entities + ddl-auto, not migration files.
...
if echo "$file_path" | grep -qE '/db/migration/|/(V[0-9]+(\.[0-9]+)*|R)__[^/]*\.sql$'; then
  echo "Flyway/Liquibase 마이그레이션 파일 생성이 차단되었습니다 (harness-workflow.md STOP §3 — 상용 보존 데이터 없음, 스키마 SSOT는 JPA 엔티티 + ddl-auto). Blocked by .claude/hooks/block-db-migration.sh — 정말 필요하면 사용자에게 먼저 확인하세요." >&2
  exit 2
fi
